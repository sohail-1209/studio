 ...
 *
 * - moderateContent - A function that moderates content based on predefined rules.
...
const moderateContentPrompt = ai.definePrompt({
  ...
  prompt: `You are an AI content moderator. Your job is to determine whether the given content violates the following ruleset:\n\nRuleset: {{{ruleset}}}\n\nContent type: {{{contentType}}}\n\nContent: {{{content}}}\n\nBased on the ruleset, determine if the content is safe. If it is not safe, explain why. Return a JSON object with 'isSafe' set to true or false, and 'reason' explaining if it is unsafe..\n\nConsiderations:\n*   Prioritize user safety and well-being.
